/*
Beautiful explanation given in top solution:
...
out of the sequence, we could enumerate each number i in the 
sequence, and use the number as the root, naturally, the subsequence
1…(i-1) on its left side would lay on the left branch of the root,
and similarly the right subsequence (i+1)…n lay on the right branch
of the root. We then can construct the subtree from the subsequence
recursively. Through the above approach, we could ensure that the BST
...
construct a BST out of a sequence of length 1 (only a root) or 0 (empty tree).
i.e.
...
Combining the above two formulas, we obtain the recursive formula 
for G(n). i.e.
